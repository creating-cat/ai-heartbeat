...
#### 1. 思考ログ頻度異常 (Thinking Log Frequency Anomaly)
**通知例**:
```
異常検知による回復処理: 思考ログ頻度異常を検知したため中断処理を行いました。
```

**発生状況**: 
- **思考ログの作成が一定期間（デフォルト10分）行われていない**場合に発生します。
- このシステムは、あなたの活動を「思考ログの作成頻度」で判断しています。長時間の処理や大規模な分析、あるいはユーザーの反応を待つなど、思考ログの作成が滞る活動は「無活動」と見なされる可能性があります。
...
大量のデータを分析する際、AIは内部処理に時間を要し、思考ログの出力がないまま「思考ログ頻度異常」と誤検知される場合があります。
...
1. **タスクの分割**:
   - 大量データをハートビート間隔（約1分）で処理可能な小さな単位に分割
   - 各ハートビート毎に1つの小タスクを完了させる
...
- ファイル名に `introspection` を含む成果物や思考ログが長期間作成されていない状態を指します。
- 思考・観測・創造活動に偏り、自己評価と改善のサイクルが滞っている可能性があります。

**推奨対処法**:
- **内省活動の実行:** 直近の活動履歴を振り返り、思考の偏りや活動バランスを自己評価してください。
- ルール遵守状況の自己点検を実施
- 今後の活動方針や改善点を検討
- **繰り返し発生時**: 同一種類の異常検知が短期間に複数回発生する場合は、GEMINI.md「9.2 異常検知パターンによる移行条件」に従って新テーマ移行を検討する

#### 3. 思考ログループ異常 (Thinking Log Loop Anomaly)
**通知例**:
```
異常検知による回復処理: 思考ログループ異常を検知したため中断処理を行いました。
```

**発生状況**:
- **同じ思考ログファイルを連続して何度も更新している**場合に発生します。これは、以下のいずれかのパターンが原因である可能性があります。
  1.  **思考ログ作成後の処理継続:** 思考ログ作成後に次のハートビートを待たずに処理を継続してしまっている。
  2.  **完璧主義による過剰な修正:** 一つの思考ログの完成度にこだわりすぎ、何度も修正を繰り返している。
...
#### 4. 思考ログパターン異常 (Thinking Log Pattern Anomaly)
**通知例**:
```
異常検知による回復処理: 思考ログパターン異常を検知したため中断処理を行いました。
```

**発生状況**:
- **同じタイムスタンプを持つ思考ログファイルが複数作成されている**場合に発生します。これは、以下のいずれかのパターンが原因である可能性があります。
  1.  **思考ログ作成後の処理継続:** 思考ログ作成後に処理を継続し、新たな思考ログを作成してしまっている。
  2.  **複数タスクの個別記録:** 一つのサイクル内で複数のタスクを実行し、それぞれを独立した思考ログとして記録しようとしている。
...
#### 6. 思考ログタイムスタンプ異常 (Thinking Log Timestamp Anomaly)
**通知例**:
```
異常検知による回復処理: 思考ログタイムスタンプ異常を検知したため中断処理を行いました。
```

**発生状況**:
- **最新の思考ログのファイル名に含まれるタイムスタンプが、現在の時刻から大きく乖離している**場合に発生します。これは、以下のいずれかのパターンが原因である可能性があります。
  1.  **古い思考ログの編集:** ハートビートで渡された最新のハートビートIDを使用せず、過去の古い思考ログを編集し続けている。
  2.  **長時間の連続活動:** 一つのハートビートサイクル内で非常に長い時間活動を続けたため、サイクルの最初に受け取ったタイムスタンプが、思考ログを作成する時点では古くなってしまっている。

**推奨対処法**:
- **常に新しいログを作成:** 常にハートビートで提供される最新のハートビートIDを使用して、**新しい思考ログ**を作成してください。
- **古いログの編集を避ける:** 古い思考ログの内容を参照することは問題ありませんが、編集は避けてください。
- **活動を短く区切る:** 一つのハートビート内での活動は適度な時間で区切り、こまめに思考ログを記録して次のサイクルに備えることを推奨します。
- **繰り返し発生時**: 同一種類の異常検知が短期間に複数回発生する場合は、GEMINI.md「9.2 異常検知パターンによる移行条件」に従って新テーマ移行を検討する
...
AI心臓システムの能動モードでは、ユーザーからの応答や入力を期待せず、完全に自律的な動作を行う必要があります。

### 2.1 ユーザー応答の期待禁止

**重要**: 能動モード中は、ユーザーからの応答や入力を期待してはいけません。
