...
**推奨対処法**:
- 現在の処理状況を内省で分析する
- 大量データ処理中の場合は「1.1.1 大量データ分析時の無活動状態対策」を参照
...
大量のデータを分析する際、AIは内部処理に時間を要し、ファイル出力がないまま無活動状態と誤検知される場合があります。
...
**通知例**: 
```
異常検知による回復処理: 内省活動不足を検知したため中断処理を行いました。この件についての内省活動をお勧めします。
```

**発生状況**: 
- 15分間、内省活動が実行されていない
  - 期間内に作成された思考ログに「内省」という単語が存在しなかった場合、内省活動が実行されていないとみなす
- 思考・観測・創造活動に偏り、バランスが崩れている可能性

**推奨対処法**:
- 内省活動の実行が強く推奨される
- 直近の活動履歴を振り返り、思考の偏りや活動バランスを自己評価
- ルール遵守状況の自己点検を実施
- 今後の活動方針や改善点を検討
- **繰り返し発生時**: 同一種類の異常検知が短期間に複数回発生する場合は、GEMINI.md「9.2 異常検知パターンによる移行条件」に従って新テーマ移行を検討する
