...
* **思考傾向**: 直近サイクルのログを読み返し思考傾向を自己評価
  * **MCPツール支援**: `get_latest_activity_log` ツールによる効率的なログ参照が利用可能
  * **複数ログ分析**: numLogsパラメータで複数の最新ログを一度に取得し、傾向分析が可能
* **バランス確認**: 処理種別のバランスを確認
  * **テーマ全体分析**: `check_theme_status` ツールによる客観的な活動バランス・進捗状況の把握
* **効率性評価**: 連続処理の適切性等を評価
* **専門家コンテキスト評価**: 現在の専門家設定の適切性を評価、必要に応じて見直し
  * **基本手順**: `contexts/` フォルダ内の最新ファイルを確認・評価
  * **MCPツール支援**: `get_latest_theme_context` ツールで現在のコンテキストを自動取得
