...
#### 7. 内省義務違反 (Introspection Obligation Violation)
...
- 深い作業完了後の最初の活動ログが内省以外の活動だった場合

**推奨対処法**:
- **内省活動の実施**: 深い作業完了後は必ず内省活動を行う
- **作業振り返り**: 完了した深い作業の成果と課題を整理
- **学習効果の定着**: 作業中に得られた洞察や学習を記録
- **例外処理の理解**: 緊急対応（エラー、バグ修正等）は例外として認められる

#### 8. flexibleモードチェックポイント不足 (Flexible Mode Checkpoint Requirement)

**通知例**:
```
flexibleモード警告: 10分間チェックポイントログが作成されていません。
```

**発生状況**: 
- flexibleモードの深い作業宣言中にチェックポイントログが10分間作成されていない場合
- 長時間の処理中でも意識的な活動の継続を示す必要がある場合

**推奨対処法**:
- **checkpointツールの使用**: 適度な間隔でcheckpointツールを使用
- **進捗の記録**: 長時間の処理でも、中間的な進捗や考察をチェックポイントとして記録
- **活動の可視化**: チェックポイントにより、深い作業中でも意識的に活動していることを示す
- **推奨間隔**: 10分以内に1回のチェックポイント作成を推奨
