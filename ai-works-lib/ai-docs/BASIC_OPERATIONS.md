...
- 原則として、テーマ成果物やプロジェクト関連ファイルの作成または更新は、一回の活動サイクルで1ファイルのみ
...
- **異常検知の回避**: 活動ログ頻度異常検知（10分）を避けるための自律的な時間管理が重要です。
...
- **結果**: 数分の処理でも複数ハートビートを見逃し、「活動ログ頻度異常」として回復処理がトリガーされてしまう可能性があります
