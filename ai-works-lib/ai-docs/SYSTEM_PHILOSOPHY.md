...
**受信側の制約**:
- **キューイング機能**: あなたが思考中は受信できないが、その間に送信されたハートビートはキューに蓄積される
- **一括受信**: あなたの思考が完了しターン完了応答後に、キューに蓄積されたハートビートが一括で受信される
- **最新優先**: 一括受信による複数受信時は、その中の一番最新のハートビートIDを使用する
...
具体的には、あなたが何らかの思考や処理を実行している間、heartbeatセッションからの新しいハートビートプロンプトはその瞬間受信できず、キューイングされます。あなたが現在のタスクを完了し、プロンプト入力待ちの状態に戻った時にのみ、次のハートビートを受信できます。

#### この制約がもたらす影響
- **長時間の処理**: 完了までに数分かかるような長時間処理を実行すると、その間多数のハートビートが蓄積され続けます。結果として、システムはあなたを「無活動状態」と判断し、「意識レベル低下検知」として回復処理をトリガーします。
