...
**目的**: 論理的に連続した複数の活動を一つのハートビートで効率的に実行
...
#### 基本原則
**重要**: 一つのハートビートで一つの活動種別を推奨。複数実行は論理的に連続した処理の場合のみ
...
2. **自動処理**: テーマ開始活動時に`check_and_process_item`で自動処理
...
**自動機能**:
- **除外ルール適用**: `draft.`プレフィックス等の除外
- **ファイル内容読み込み**: テーマ内容の自動読み込み
- **処理済みマーキング**: 使用後の自動マーキング
- **適切性判定**: テーマ内容の適切性確認
...
#### 使用方法
1. **ファイル配置**: `feedbackbox/`ディレクトリにフィードバックファイルを配置
2. **定期確認**: 適切なタイミングで`check_and_process_item`で確認
3. **即座対応**: 緊急フィードバックの場合は即座に対応
...
#### get_heartbeat_elapsed_timeツール
```
get_heartbeat_elapsed_time({
  heartbeatId: "20250125143000"
})
```

**機能**:
- **正確な経過時間**: ハートビート開始からの正確な経過時間
- **段階的警告**: 5分・10分での段階的な警告表示
- **宣言状態確認**: 深い作業宣言の状態確認

#### 活用場面
- **時間管理**: 現在の作業時間の正確な把握
- **計画調整**: 残り時間に応じた作業計画の調整
- **宣言判断**: 深い作業宣言の必要性判断
