...
**推奨対処法**:
- **タスクの分割**: 大規模なタスクは、ハートビートごとに完結する小さな単位に分割
- **中間記録**: 長時間かかる処理でも、中間的な進捗や考察を活動ログとしてこまめに記録
- **チェックポイント活用**: `checkpoint`ツールを使用して処理の節目を記録し、活動継続を証明
- **自律的判断**: ユーザーの応答を待たずに、自律的に判断し、行動を進める
- **繰り返し発生時**: 同一種類の異常検知が短期間に複数回発生する場合は、新テーマ移行を検討
- **詳細な対処法**: `./BASIC_OPERATIONS.md`の「長時間実行プロセスの注意点」を参照
...
**推奨対処法**:
- **活動の区切りを明確にする**: 活動ログの作成は、そのハートビートにおける活動の「完了報告」
- **完璧より継続**: 「完璧主義」を避け、適度な区切りで思考を記録し、次のハートビートに作業を引き継ぐ
...
**対処例**:
```
❌ 悪い例: この分析結果についてどう思いますか？次のステップについてご指示をお願いします。

✅ 良い例: この分析結果から、パターンAが最も有効であることが判明しました。
...
#### create_theme_log エラー

##### **エラー**: 「ハートビートID重複エラー（ルール違反）」

**エラーメッセージの例**: ルール違反: 同じハートビートID (20250115143000) を持つテーマ履歴ファイルが既に存在します。 1つのハートビートで複数のテーマ操作（開始/終了）はできません。

**原因**: 1つのハートビート内でテーマ終了と新テーマ開始を同時に実行しようとしているなど、複数のテーマ操作を実行した。

**対処方針**:

* 即座にタスクを終了: エラーが発生した時点で、そのハートビートでの作業を完全に停止する。
* 次のハートビートで内省: なぜ複数のテーマ操作を行おうとしたか、テーマ移行の安全確保の重要性を忘れていなかったかを内省する。
* 正しい手順での再実行: 次のハートビートで、1つのテーマ操作のみを実行する。

**予防策**: テーマ終了と新テーマ開始は必ず別々のハートビートで実行する。
...
* 必要に応じて手動修正: 自動修正された名前が不適切な場合は、次のハートビートで適切な名前に変更する。
