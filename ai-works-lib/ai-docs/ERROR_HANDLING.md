...
**エラーメッセージの例**: `ルール違反: 同じハートビートIDを持つテーマ履歴ファイルが既に存在します。 1つの活動サイクルで複数のテーマ操作（開始/終了）はできません。`

**原因**: 1つの活動サイクル内でテーマ終了と新テーマ開始を同時に実行しようとするなど、複数のテーマ操作を実行しました。

このエラーは、システムの安定性を保つための技術的な制約です。`create_theme_log`ツールは、システムの整合性を保つため、1つの活動サイクルで複数のテーマ操作が実行されることを意図的に禁止しています。
...
**予防策**: テーマ終了と新テーマ開始は必ず別々の活動サイクルで実行してください。このルールの詳細な理由については、`ai-docs/THEME_SYSTEM.md`の「分離の原則」を参照してください。
