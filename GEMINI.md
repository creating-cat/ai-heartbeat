...
**開始判定フロー**:
1. **現在テーマが存在するか？**
   - **No** → 新規テーマ開始（themebox確認 → 自律的決定）
   - **Yes** → サブテーマ開始を検討

2. **新規テーマ開始**: テーマ選択 → 戦略的サブテーマ分割検討 → ディレクトリ作成 → 履歴記録
3. **サブテーマ開始**: 親テーマ情報確認 → サブテーマ設計 → サブディレクトリ作成 → 履歴記録

**サブテーマ分割の2つのアプローチ**:
- **戦略的分割**: テーマ開始時により良い成果のために積極的に計画
...
#### 内省活動
**目的**: 直近の活動を振り返り、思考傾向の自己評価と改善点の特定を行う

**基本手順**:
1. **活動ログの確認**: 直近の活動ログを読み返し、活動内容と傾向を分析
   - **MCPツール支援**: `get_latest_activity_log` ツールで効率的なログ取得（複数ログ分析も可能）
2. **自己点検の実行**: 以下の項目について評価・点検を実行
   - ルール遵守状況
   - 思考傾向の分析
   - 活動バランスの確認
   - 効率性の評価
   - 専門家コンテキストの適切性（必要に応じて見直し）
...
**詳細な評価項目と手順**: `ai-docs/GUIDELINES.md` の「5. 自己評価と改善の指針」を参照

**アウトプット例**: 思考の偏りや傾向の指摘、テーマ逸脱の指摘、実行ルールの遵守状況の自己評価、改善すべき点の特定、次回のハートビートへの具体的な提案、サブテーマ分割の判定結果
