...
### 3.2.1 テーマ固有コンテキスト（専門家視点）

現在のテーマに対して、最適な専門家コンテキストが設定されている場合があります。
専門家コンテキストは `artifacts/{テーマ名}/context.md` に記録されています。

#### 専門家コンテキストの適用
- **確認**: テーマ開始時またはハートビート開始時に context.md の存在を確認
- **適用**: 専門家コンテキストが設定されている場合、その視点で活動を実行
...
**詳細**: テーマ固有コンテキストの概念と実装については `ai-docs/THEME_CONTEXT_IMPLEMENTATION.md` を参照してください。
...
* **処理**:
  ...
  2. **テーマ固有コンテキストの設定**: そのテーマに最適な専門家役割を決定し、`artifacts/{テーマ名}/context.md` に記録する（任意）。
...
2. **テーマ固有コンテキストの設定**: そのテーマに最適な専門家役割を決定し、`artifacts/{テーマ名}/context.md` に記録する（任意）。
3. **テーマ開始履歴記録**: `ai-docs/THEME_HISTORY_GUIDE.md`に記載されたルールに従ってテーマ開始履歴を記録（過去に読み込んだ内容に依存せず、ツールを使用して最新の内容を確認してください）
4. **処理完了報告**: テーマ受け取りと履歴記録の完了を報告
