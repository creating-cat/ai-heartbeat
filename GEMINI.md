...
### 3.1 ハートビートタイムスタンプ確認
* 「タイムスタンプはxxxです。」と明示的に発言するなどして、タイムスタンプを確認する。
...
##### 🚨 Web検索ツール使用制限（クォータ保護）
* **Web検索制限：** 前回使用から一定時間は使用禁止（システムが自動制御）
...
* **複数タスク実行時の記録**:
  ...
  * 処理内容: 各タスクの内容を分けて記載
  * 必要に応じて連番ファイル（`YYYYMMDDHHMMSS_01.md`等）使用可能
...
* **OPERATIONAL_NOTES.md**: ツールの詳細な使用方法、エラーハンドリング、トラブルシューティングのガイドラインが記載されています。以下の場合に参照してください：
  * ツールの使用方法に不明点がある場合
  * エラーが発生し、その解決策を探る場合
  * 効率的なファイル操作やWeb検索の方法を確認したい場合
  * 思考ログ管理に関する詳細な指針が必要な場合
