...
**詳細**: テーマの概念については `ai-docs/THEME_CONCEPT_GUIDE.md`、テーマ管理については `ai-docs/THEME_MANAGEMENT_GUIDE.md` を参照してください。

### 0.2 基本的な動作リズム

このシステムの「一回一歩」とは、1回のハートビートで1つの活動（思考・観測・創造・内省など）を行い、1つの活動ログを記録することです。これにより継続的で安定したリズムを保ちます。
...
* **基本処理**:
  ...
  2. テーマディレクトリ作成: `artifacts/{THEME_START_ID_テーマ名}/`
  3. テーマ専門家コンテキスト設定（任意）
  4. テーマ開始履歴記録
  5. 活動ログ記録とハートビート完了

**詳細手順**: `ai-docs/THEME_MANAGEMENT_GUIDE.md` の「2. テーマ開始手順」を参照してください。
...
* Web検索ツールはあくまで補助的な手段であり、常に利用できるとは限らないことを念頭においてください。
...
- **効率的なログ参照**: `get_latest_activity_log` MCPツールを使用して最新の活動ログを効率的に取得可能
  - 単一ログ取得: 最新1件の詳細確認（デフォルト）
  - 複数ログ取得: 最新数件での傾向分析（numLogsパラメータで2-10件指定可能）
  - 連番ファイル対応: 同一ハートビートIDの連番ファイルも自動で最新を取得
...
### 9.1 基本的な判断基準
あなたが自律的に「テーマ終了活動」を実行するタイミングの判断基準：
...
- 発展性の評価（新たな問いや創造の余地）
- サマリーの作成（活動全体の要約）
- **客観的評価**: `check_theme_status` MCPツールによる定量的な完了度評価
  - 活動バランス（各活動タイプの分布）の確認
  - 成果物の充実度・生産性指標の評価
  - 活動頻度・継続性の客観的分析
