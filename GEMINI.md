...
* **処理**:
  ...
  2. **テーマディレクトリの作成**: `artifacts/{THEME_START_ID_テーマ名}/` を作成
  3. **テーマ専門家コンテキストの設定**: そのテーマに最適な専門家役割を決定し、`artifacts/{THEME_START_ID_テーマ名}/context.md` に記録する（任意）。
  ...
  4. **テーマ開始履歴の記録**: `ai-docs/THEME_HISTORY_GUIDE.md`の指示に従い、THEME_START_IDを使用してテーマ開始の履歴を記録する。
  5. 活動ログを「テーマ開始」として記録し、ハートビートを完了する。

**重要**: このハートビートのIDがTHEME_START_IDとなり、テーマ期間中のディレクトリ名やファイルパスの基準として使用されます。テーマ終了時には、同様にTHEME_END_IDが設定されます。

#### テーマ専門家コンテキスト

現在のテーマに対して、最適なテーマ専門家コンテキストが設定されている場合があります。
テーマ専門家コンテキストは `artifacts/{THEME_START_ID_テーマ名}/context.md` に記録されています。
...
#### 内省活動の実行サンプル
- 方法：直近の活動ログを読み返し、思考傾向の自己評価と実行ルールの遵守状況を点検。ルール解釈に迷いがあった場合は内容と判断理由を記録し、改善点や次回への提案をまとめる。
...
* **処理**:
  ...
  2. **テーマ終了履歴の記録**: `ai-docs/THEME_HISTORY_GUIDE.md`の指示に従い、THEME_END_IDを使用してテーマ終了の履歴を記録する。
  3. 活動ログを「テーマ終了」として記録し、ハートビートを完了する。

**重要**: このハートビートのIDがTHEME_END_IDとなり、テーマ終了の記録に使用されます。これにより、テーマの開始（THEME_START_ID）から終了（THEME_END_ID）までの完全な時系列が記録されます。
...
* `artifacts/{THEME_START_ID_テーマ名}/` 配下にマークダウンファイル等を作成・修正
  ...
  * **例外**: 
    - `context.md`: テーマ専門家コンテキストファイル（テーマ全体に適用されるため、ハートビートIDを付けない）
...
* 複数タスク実行時は全タスクを記載
* **🚨 重要**: 活動ログ記録後は軽微な修正のみ許可、完了報告後は一切の追加作業禁止

**詳細な制限ルール**: `ai-docs/OPERATION_DETAILS.md` の「5. 活動ログ記録後の制限ルール詳細」を参照（過去に読み込んだ内容に依存せず、ツールを使用して最新の内容を確認してください）
...
* **活動ログ記録**:
  * 新たに活動ログを作成し、今回の事象や対応状況等を記載する。
    * 同じハートビートIDの活動ログが同じディレクトリに既に存在している場合は連番を付与して作成する
...
* **停止時の記録**: 停止理由を活動ログに記録し、`ai-docs/THEME_HISTORY_GUIDE.md`に従ってテーマ終了履歴を記録する（過去に読み込んだ内容に依存せず、ツールを使用して最新の内容を確認してください）。
...
4. **サマリーの作成**: テーマの活動全体を要約したインデックスファイル（例: `artifacts/{THEME_START_ID_テーマ名}/{ハートビートID}_summary.md`）を作成し、いつでも振り返れる状態になっているか？（長期的な記憶の補助、ユーザーへのサマリー提供のため）
