...
create_introspection_obligation() {
    local completed_heartbeat_id="$1"
    local obligation_file="ai-works/stats/introspection_obligation/${completed_heartbeat_id}.txt"
    
    # ディレクトリが存在しない場合は作成
    mkdir -p "ai-works/stats/introspection_obligation"
    
    # 義務状態ファイルを作成
    echo "深い作業完了時刻: $(date '+%Y%m%d%H%M%S')" > "$obligation_file"
    echo "完了ハートビートID: $completed_heartbeat_id" >> "$obligation_file"
    
    debug_log "INTROSPECTION_OBLIGATION: Created obligation for $completed_heartbeat_id"
}
...
clear_introspection_obligation() {
    ...
    if [ -f "$obligation_file" ]; then
        rm "$obligation_file"
        debug_log "INTROSPECTION_OBLIGATION: Cleared obligation for $completed_heartbeat_id"
    fi
}

# 例外キーワードによる内省義務の例外判定
is_introspection_exception() {
    local activity_log_file="$1"
    
    # 例外キーワードの配列
    local exception_keywords=("緊急" "エラー" "修正" "バグ" "障害" "interrupt" "割り込み")
    
    for keyword in "${exception_keywords[@]}"; do
        if grep -q "$keyword" "$activity_log_file" 2>/dev/null; then
            debug_log "INTROSPECTION_OBLIGATION: Exception keyword '$keyword' found in $(basename "$activity_log_file")"
            return 0  # 例外として認める
        fi
    done
...
# 深い作業完了を検知して内省義務を作成する関数
detect_deep_work_completion() {
    # 新しく作成された .completed.txt ファイルを検索
    local completed_files=$(find ai-works/stats/deep_work -name "*.completed.txt" -type f 2>/dev/null)
    
    if [ -z "$completed_files" ]; then
        debug_log "INTROSPECTION_OBLIGATION: No completed deep work files found"
        return 0
    fi
    
    for completed_file in $completed_files; do
        local completed_heartbeat_id=$(basename "$completed_file" .completed.txt)
        local obligation_file="ai-works/stats/introspection_obligation/${completed_heartbeat_id}.txt"
        
        # まだ義務状態が作成されていない場合のみ作成
        if [ ! -f "$obligation_file" ]; then
            create_introspection_obligation "$completed_heartbeat_id"
            debug_log "INTROSPECTION_OBLIGATION: New obligation created for completed deep work: $completed_heartbeat_id"
        fi
    done
}
...
# 引数: current_time
# 戻り値: 常に0（エラーコードはecho出力に含める）
# 出力: "error_code:detail" 形式（0:0=正常, 2:heartbeat_id=違反）
check_introspection_obligation_violation() {
    local current_time="$1"
    
    debug_log "INTROSPECTION_OBLIGATION check started: current_time=$current_time"
    
    # 内省義務状態ファイルを検索
    local obligation_files=$(find ai-works/stats/introspection_obligation -name "[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].txt" -type f 2>/dev/null)
    
    if [ -z "$obligation_files" ]; then
        debug_log "INTROSPECTION_OBLIGATION: No active obligations"
        echo "0:0"
        return 0
    fi
    
    # 各義務状態をチェック
    for obligation_file in $obligation_files; do
        local obligation_heartbeat_id=$(basename "$obligation_file" .txt)
        debug_log "INTROSPECTION_OBLIGATION: Checking obligation for $obligation_heartbeat_id"
        
        # この義務状態以降に作成された活動ログを検索
        local obligation_time=$(convert_timestamp_to_seconds "$obligation_heartbeat_id")
        
        if [ -z "$obligation_time" ]; then
            debug_warning "INTROSPECTION_OBLIGATION: Invalid heartbeat ID format: $obligation_heartbeat_id"
            continue
        fi
        
        # obligation_time以降の活動ログを取得
        local recent_activity_logs=""
        while IFS= read -r file; do
            if [ -f "$file" ]; then
                ...
                if [[ "$filename" =~ ^([0-9]{14}) ]]; then
                    ...
                    if [ -n "$file_time" ] && [ $file_time -gt $obligation_time ]; then
                        if [ -z "$recent_activity_logs" ]; then
                            recent_activity_logs="$file_time $file"
                        else
        ...
        done < <(find ai-works/artifacts -path "*/histories/*.md" -name "[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]*.md" -type f 2>/dev/null)
        
        if [ -n "$recent_activity_logs" ]; then
            # 最初の活動ログ（義務状態直後の活動ログ）をチェック
            local first_log=$(echo "$recent_activity_logs" | sort -n | head -n 1)
            local first_log_file=$(echo "$first_log" | cut -d' ' -f2-)
            
            debug_log "INTROSPECTION_OBLIGATION: Checking first activity log after obligation: $(basename "$first_log_file")"
            
            # ファイル内容に「内省」が含まれているかチェック
            if grep -q "内省" "$first_log_file" 2>/dev/null; then
                # 内省が行われている場合、義務状態をクリア
                clear_introspection_obligation "$obligation_heartbeat_id"
                debug_log "INTROSPECTION_OBLIGATION: Obligation fulfilled for $obligation_heartbeat_id"
            elif is_introspection_exception "$first_log_file"; then
                # 例外として認められる場合、義務状態をクリア
                clear_introspection_obligation "$obligation_heartbeat_id"
                debug_log "INTROSPECTION_OBLIGATION: Obligation cleared due to exception for $obligation_heartbeat_id"
            else
                # 内省が行われておらず、例外でもない場合、違反として検知
                debug_warning "INTROSPECTION_OBLIGATION: Violation detected for $obligation_heartbeat_id"
                echo "2:$obligation_heartbeat_id"
                return 0
            fi
        else
            debug_log "INTROSPECTION_OBLIGATION: No activity logs found after obligation for $obligation_heartbeat_id"
        fi
    done
    
    debug_log "INTROSPECTION_OBLIGATION: All obligations checked, no violations"
    echo "0:0"
    return 0
}