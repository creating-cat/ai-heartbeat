...
readonly ADVICE_ACTIVITY_LOG_PATTERN="
同じタイムスタンプを持つ活動ログファイルが複数作成されています。これは、以下のいずれかのパターンが原因である可能性があります。

1.  **活動ログ作成後の処理継続:**
    活動ログを作成してハートビートのタスクを完了したにもかかわらず、次のハートビートを待たずに処理を継続し、新たな活動ログを作成してしまっているようです。
    運用ガイドラインの「一回一歩」の原則に基づき、活動ログを作成した後は、そのサイクルでの活動は完了とみなし、静かに次のハートビートを待つ必要があります。

2.  **複数タスクの個別記録:**
    一つのハートビートサイクル内で複数のタスクを実行し、それぞれを独立した活動ログとして記録しようとしている可能性があります。
